...
;;;;;;;;;;;;;;;;;;;;;;;;
extensions [ rnd ]
turtles-own[n p]  ;; n is the accumulated points; p is the probability of cooperate.
to setup
  clear-all
...
to prisoner-dilemma
  ...
  let p3 pb * (1 - pa)
  let p4 (1 - pa) * (1 - pb)
  let probabilities (list p1 p2 p3 p4)
  let po1 (list 3 3)
  let po2 (list 1 5)
  let po3 (list 5 1)
  let po4 (list 2 2)
  let payoff (list po1 po2 po3 po4)
  let outcome first rnd:weighted-one-of-list (map list payoff probabilities) last
  ask end1 [
    set n n + first outcome
  ]
  ask end2 [
    set n n + last outcome
  ]
end

to-report find-highest
  report max-one-of turtles [n]
end

to-report find-nicest
  report max-one-of turtles [p]
end
...
to-report find-partner
  ...
  let q 1 - prob
  let probs (list .9 .1)
  report first rnd:weighted-one-of-list (map list (list find-connected find-highest) probs) last
...
"" ""
PENS
"default" 1.0 2 -16777216 true "" "if not plot? [ stop ]\nlet max-wealth max [n] of turtles\nlet min-wealth min [n] of turtles\nlet max-connect max [count link-neighbors] of turtles\nplot-pen-reset  ;; erase what we plotted before\nset-plot-x-range 0 (max-wealth + 1)\nset-plot-y-range 0 (max-connect + 1)\nplot-pen-reset  ;; erase what we plotted before\nask turtles [plotxy n count link-neighbors]"
...
"" ""
PENS
"default" 1.0 2 -16777216 true "" "ask turtles [plotxy p n]"
